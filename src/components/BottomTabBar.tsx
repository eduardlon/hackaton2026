    ...
    const timeout = setTimeout(() => {
      ...
      askFinancialChat(QUICK_AI_PROMPT)
        .then((res) => {
          ...
          setAnswer(text);
          setStatus('answer');
          Speech.speak(text, { language: 'es-CO', rate: 0.95 });
        })
        .catch(() => {
          const fallback =
          ...
          setAnswer(fallback);
          setStatus('answer');
          Speech.speak(fallback, { language: 'es-CO', rate: 0.95 });
        });
    }, 1100);
        ...
        >
          ...
          <View style={{ alignItems: 'center', gap: 14, paddingVertical: 6 }}>
            ...
            <Text variant="bodySmall" tone="muted" align="center">
              {status === 'answer'
                ? answer
                : 'Mantén presionado el botón IA para abrir una recomendación inmediata. La respuesta se lee en voz alta.'}
            </Text>
          </View>
        </View>
      </View>
