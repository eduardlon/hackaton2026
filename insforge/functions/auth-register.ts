module.exports = async function(request) {
  ...
  try {
    ...
    if (!name || name.length < 2) {
      return json({ error: { code: 'NAME_REQUIRED', message: 'Ingresa tu nombre para crear la cuenta.' } }, 400, corsHeaders);
    }
    ...
    if (existingError) {
      return json({ error: { code: 'LOOKUP_FAILED', message: 'No se pudo validar si el celular ya existe.' } }, 500, corsHeaders);
    }

    if (existing) {
      return json({ error: { code: 'PHONE_ALREADY_EXISTS', message: 'Ese celular ya tiene una cuenta. Inicia sesión con tu PIN.' } }, 409, corsHeaders);
    }
    ...
    const { data: users, error: createError } = await client.database
      .from('phone_users')
      .insert({
        id: userId,
        name,
        phone,
        type: body.userType || 'customer',
        pin_hash: pinHash,
        updated_at: now
      })
      .select();

    if (createError || !users?.[0]) {
      return json({ error: { code: 'USER_CREATE_FAILED', message: createError?.message || 'No se pudo crear tu cuenta.' } }, 500, corsHeaders);
    }

    await ensureFinancialProfile(client, users[0]);

    return json(await buildAuthResponse(users[0], 'phone-pin-register'), 200, corsHeaders);
  } catch (error) {
    return json({ error: { code: 'INTERNAL_ERROR', message: error?.message || 'Error inesperado creando cuenta.' } }, 500, corsHeaders);
  }
};
